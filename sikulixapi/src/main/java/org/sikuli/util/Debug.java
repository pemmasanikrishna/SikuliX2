 ...
 * (which is equivalent to using SX.Debuglogs = false)<br> <br> It prints if the level
...
//</editor-fold>
  ...
  public static boolean setLogFile(String fileName) {
    ...
    if (fileName != null) {
      if ("".equals(fileName)) {
        fileName = new File(System.getProperty("user.dir"), "SikulixLog.txt").getAbsolutePath();
      }
      try {
        logfile = fileName;
        if (printout != null) {
  ...
  /**
   * Sikuli messages from actions like click, ...<br> switch on/off: SX.ActionLogs
   *
  ...
  /**
   * informative Sikuli messages <br> switch on/off: SX.InfoLogs
   *
   * @param message String or format string (String.format)
   * @param args    to use with format string
   */
  ...
  /**
   * Sikuli debug messages with default level<br> switch on/off: SX.DebugLogs (off) and/or
   * -Dsikuli.Debug
   *
   * @param message String or format string (String.format)
   * @param args    to use with format string
  ...
  /**
   * messages given by the user<br> switch on/off: SX.UserLogs<br> depending on
   * SX.UserLogTime, the prefix contains a timestamp <br> the user prefix (default "user")
...
//TODO replace the hack -99 to filter user logs
        log(-99, String.format("%s (%s)",
                SX.UserLogPrefix, df.format(new Date())), message, args);
      } else {
        log(-99, String.format("%s", SX.UserLogPrefix), message, args);
      }
    }
  }

  /**
   * Sikuli debug messages with level<br> switch on/off: SX.DebugLogs (off) and/or
   * -Dsikuli.Debug
   *
   * @param level   value
   * @param message String or format string (String.format)
...
//TODO replace the hack -99 to filter user logs
    ...
    if (level <= DEBUG_LEVEL) {
      ...
      if (level > -99) {
        isRedirected = doRedirect(CallbackType.DEBUG, prefix, sout, (Object) null);
      } else if (level == -99) {
        isRedirected = doRedirect(CallbackType.USER, prefix, sout, (Object) null);
      }
      if (!isRedirected) {
  ...
  /**
   * Sikuli profiling messages<br> switch on/off: SX.ProfileLogs, default off
   *
  ...
  /**
   * start timer
   * <br>log output depends on SX.ProfileLogs
   *
   * @return timer
   */
