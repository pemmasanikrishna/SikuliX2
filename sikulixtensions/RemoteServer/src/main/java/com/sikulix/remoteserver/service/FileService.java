...
public class FileService {
    ...
    public Response uploadFile(final FormDataMultiPart form, @QueryParam("saveTo") final String saveToPath) {
        ...
        for (FormDataBodyPart filePart : form.getFields("file")) {
            filePath = (saveToPath.endsWith("\\") ? saveToPath : saveToPath.concat("\\"))
                    + filePart.getContentDisposition().getFileName();

            try {
                inputStream = filePart.getValueAs(InputStream.class);
                outputStream = new FileOutputStream(new File(separatorsToSystem(filePath)));
    ...
    public StreamingOutput downloadFile(@QueryParam("fromPath") final String fromPath) {
        return new StreamingOutput() {
            ...
            public void write(final OutputStream outputStream) {
                ...
                try {
                    inputStream = new FileInputStream(new File(separatorsToSystem(fromPath)));
                    IOUtils.copy(inputStream, outputStream);
                    outputStream.flush();
