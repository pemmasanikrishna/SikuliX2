...
public class SXCaptureHook {
  ...
  private void init(NativeHook theHook) {
    hook = theHook;
    ...
    NativeHookCallback cbCapture = new NativeHookCallback() {
      public void callback(NativeInputEvent evt) {
        if (hook.startGesture()) {
          shot.clear();
          shot.add(0, SXCaptureHook.getScreenImage(hook));
          ...
          if (!SX.isNull(event)) {
            ...
            Runnable doit = new Runnable() {
              public void run() {
                ...
                try {
                  make(hook, mevt.getX(), mevt.getY(), 0, 0, shot.get(0));
                } catch (Exception ex) {
  ...
  private class MakeCapture extends MouseAdapter implements WindowListener, KeyListener {
  ...
  private static Object getScreenImage(NativeHook hook) {
    BufferedImage bImg = Do.capture().get();
    return bImg;
  }
